...
/**
 * This class implements the Dual-Pivot Quicksort algorithm by      |Dual-Pivot Quicksort排序算法，经典快排的升级版本
 * Vladimir Yaroslavskiy, Jon Bentley, and Josh Bloch. The algorithm|基准点由1个变为了两个，利用了cpu性能升级和内存速度升级
 * offers O(n log(n)) performance on many data sets that cause other|的不对等。对于相同下标并且值不变的情况，只算一次扫描
